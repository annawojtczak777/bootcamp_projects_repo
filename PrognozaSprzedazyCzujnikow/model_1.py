...
df.info() #informacje o strukturze danych

print(df.describe().T.to_string()) #opis danych liczbowych

duplikaty = df.duplicated() #sprawdzenie, czy w zbiorze danych wystepują duplikaty
df_duplikaty = df[df.duplicated()]
print('\nDuplikaty')
print(df_duplikaty)
...
df.to_excel('oczyszczone_glukometry.xlsx', sheet_name='Wyniki', index=False) #eksport oczyszczonych i uzupełnionych danych


#-------------Eksploracyjna analiza danych (EDA)-------------------------

#Grupowanie i sortowanie danych, kolejność miesięcy

print('\nEksploracja i wizualizacja zestawu danych\n')
kolejnosc = [
    'styczeń', 'luty', 'marzec', 'kwiecień', 'maj', 'czerwiec',
    'lipiec', 'sierpień', 'wrzesień', 'październik', 'listopad', 'grudzień'
]
df['Miesiąc'] = pd.Categorical(df['Miesiąc'], categories=kolejnosc, ordered=True) # Ustawienie kolejności kategorii


df_grouped = df.groupby('Miesiąc')['Liczba wyrobów'].sum().reset_index() # Grupowanie i sumowanie liczby zrefundownaych czujników wg miesiąca
print(df_grouped)


# Wizualizacja trendów
# Sumaryczna liczba refundowanych czujników glukozy w kolejnych miesiącach
plt.figure(figsize=(12, 5))
sns.lineplot(data=df_grouped, x='Miesiąc', y='Liczba wyrobów', marker='o')
plt.title('Sumaryczna liczba refundowanych czujników glukozy w kolejnych miesiącach')
plt.xlabel('Miesiąc')
plt.ylabel('Liczba czujników glukozy')
plt.xticks(rotation=45)
plt.grid(True)
plt.tight_layout()
plt.show()

# Rozkład zmiennej celu, histogram liczby zrefundowanych glukometrów
plt.figure(figsize=(10, 6))
sns.histplot(df['Liczba wyrobów'], bins=20, kde=True, color='skyblue')
plt.title('Histogram liczby zrefundowanych glukometrów')
plt.xlabel('Liczba glukometrów')
plt.ylabel('Liczba obserwacji')
plt.grid(True)
plt.tight_layout()
plt.show()

#transformacja logarytmiczna w celu skompresowania skali wartości
import numpy as np
plt.figure(figsize=(10, 6))
sns.histplot(np.log1p(df['Liczba wyrobów']), bins=20, kde=True, color='skyblue')
plt.title('Histogram zrefundowanych glukometrów po transformacji log')
plt.xlabel('Liczba glukometrów')
plt.ylabel('Liczba obserwacji')
plt.grid(True)
plt.tight_layout()
...
# Boxplot z wartościami odstającymi
plt.figure(figsize=(12, 6))
sns.boxplot(x=df['Liczba wyrobów'], color='gray')
plt.title('Boxplot dla liczby glukometrów')
plt.xlabel('Liczba glukometrów')
plt.grid(True)
plt.tight_layout()
plt.show()
...
print(df.describe().T.to_string()) #statystyki

# Grupowanie po wszystkich województwach
wojewodztwa_summary = df.groupby('Województwo świadczeniodawcy')['Liczba wyrobów']\
                       .sum().sort_values(ascending=False)

# Wykres słupkowy liczby sprzedanych wyrobów z podziałem na województwa
plt.figure(figsize=(12, 6))
sns.barplot(x=wojewodztwa_summary.values, y=wojewodztwa_summary.index, palette='Blues_r')
plt.title('Liczba sprzedanych glukometrów z podziałem na województwa')
plt.xlabel('Suma liczby glukometrów')
plt.grid(axis='x')
plt.tight_layout()
plt.show()

# Wykres słupkowy liczby sprzedanych wyrobów z podziałem na województwa, bez mazowieckiego
wojewodztwa_summary_bez_maz = df[df['Województwo świadczeniodawcy'] != 'mazowieckie'].groupby('Województwo świadczeniodawcy')['Liczba wyrobów']\
                       .sum().sort_values(ascending=False)
plt.figure(figsize=(12, 6))
sns.barplot(x=wojewodztwa_summary_bez_maz.values, y=wojewodztwa_summary_bez_maz.index, palette='Blues_r')
plt.title('Liczba sprzedanych glukometrów z podziałem na województwa bez mazowieckiego')
plt.xlabel('Suma liczby glukometrów')
plt.grid(axis='x')
plt.tight_layout()
plt.show()

# Analiza zaleznosci między zmiennymi

corr_matrix = df.select_dtypes(include='number').corr() # Obliczenie macierzy korelacji
# Rysowanie heatmapy
plt.figure(figsize=(8, 6))
sns.heatmap(corr_matrix, annot=True, cmap='Blues', fmt='.2f', square=True, linewidths=0.5)
plt.title("Heatmapa korelacji (zmienne liczbowe)")
plt.tight_layout()
plt.show()
...
df_encoded = pd.get_dummies(df, columns=['Województwo świadczeniodawcy'], drop_first=True)
...
X = df_encoded.drop(columns=['Liczba wyrobów', 'Kod produktu', 'Kod świadczeniodawcy', 'Nazwa','REGON', 'Miesiąc', 'log_Liczba_wyrobow'])
...
feature_importances = feature_importances.sort_values(ascending=False).head(5)  # top 5 najważniejszych
